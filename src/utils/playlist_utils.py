...
def add_task_to_playlist_for_ui(task_name, current_tasks):
    ...
    if not task:
        return current_tasks, f"Task '{task_name}' not found"
    
    new_task = [task["name"], task["description"]]
    
    # Handle both list and DataFrame objects
    if isinstance(current_tasks, pd.DataFrame):
        # Check if task already exists in the DataFrame
        if not current_tasks.empty and task["name"] in current_tasks.iloc[:, 0].values:
            return current_tasks, f"Task '{task_name}' is already in the playlist"
            
        # For pandas DataFrame, use concat instead of append
        new_row = pd.DataFrame([new_task], columns=current_tasks.columns)
        updated_tasks = pd.concat([current_tasks, new_row], ignore_index=True)
    else:
        # Check if task already exists in the list
        if current_tasks and any(task["name"] == t[0] for t in current_tasks if isinstance(t, (list, tuple)) and len(t) > 0):
                return current_tasks, f"Task '{task_name}' is already in the playlist"
...
async def play_playlist_for_ui(playlist_tasks, agent_type, llm_provider, llm_model_name, llm_num_ctx, llm_temperature, 
    ...
    if isinstance(playlist_tasks, pd.DataFrame):
        ...
        for _, row in playlist_tasks.iterrows():
            if len(row) > 0:
                task_name = row[0]  # Get task name from first column
                if task_name:  # Skip empty task names
                    task_list.append(task_name)
    else:
        # Handle list type
        if not playlist_tasks or len(playlist_tasks) == 0:
            yield gr.update(value="No tasks to play"), gr.update(interactive=True), gr.update(interactive=False), "Playlist is empty"
            return
