...
def create_ui(config, theme_name="Ocean"):
    ...
    with gr.Blocks(
        ...
        with gr.Tab("Agent Activity"):
            with gr.Row():
                ...
                with gr.Column(scale=3):
                    ...
                    comprehensive_export_button = gr.Button("Export Comprehensive Logs", variant="primary")
        ...
        def update_extracted_data(data_type="all", data_source="all", search_term=""):
            ...
            Returns:
                Tuple of (filtered_data_json, metadata_json, data_types, data_sources)
            """
            try:
                # Helper function for sanitizing inputs locally
                def safe_sanitize(value, default=None):
                ...
                # Format the data for display - convert to JSON strings
                formatted_data = json.dumps(filtered_data, indent=2)
                metadata_json = json.dumps(metadata)
                
                return formatted_data, metadata_json, default_types, default_sources
            except Exception as e:
        ...
        with gr.TabItem("🎥 Recordings", id=8):
            ...
            # Connect comprehensive export button
            comprehensive_export_button.click(
                fn=export_comprehensive_logs,
                inputs=[],
                outputs=[gr.Textbox(label="Export Status")]
            )
